...
import { ClipboardList, X, ChevronRight, CheckSquare, Square, Sparkles, Loader2 } from 'lucide-react';
import { RECOMENDACIONES_CATALOG } from '../../../shared/data/recomendaciones';

/**
 * RecomendacionesChecklistPanel - Panel de recomendaciones médicas
 * Checklist por categoría con generación IA
 */
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-fade-in">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100">Checklist de Recomendaciones por Categoría</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-bold">{countSelected} seleccionadas</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {Object.entries(RECOMENDACIONES_CATALOG).map(([catKey, catData]) => {
            ...
            return (
              <div key={catKey} className={`border rounded-xl overflow-hidden ${selectedInCat > 0 ? 'border-emerald-300' : 'border-gray-200'}`}>
                <button
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {selectedInCat > 0 && (
                      <span className="bg-emerald-600 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{selectedInCat}</span>
                    )}
                  </div>
                  <ChevronRight className={`w-4 h-4 transition-transform ${expandido[catKey] ? 'rotate-90' : ''}`} />
                </button>
                {expandido[catKey] && (
                    ...
                    {catData.items.map((item) => (
                      <label key={item.id}
                        className={`checklist-item flex items-start gap-2 p-2 rounded-lg cursor-pointer transition ${
                          selected[item.id] ? 'bg-emerald-50 border border-emerald-200' : 'hover:bg-gray-50'
                        }`}>
                        <div className="mt-0.5 flex-shrink-0">
                          {selected[item.id] ? <CheckSquare className="w-4 h-4 text-emerald-600" /> : <Square className="w-4 h-4 text-gray-300" />}
                        </div>
                        <input type="checkbox" className="hidden" checked={!!selected[item.id]}
                          onChange={() => onChange((p) => ({ ...p, [item.id]: !p[item.id] }))} />
                        <p className={`text-xs leading-relaxed ${selected[item.id] ? 'text-emerald-800 font-medium' : 'text-gray-700'}`}>
                          {item.texto}
                        </p>
                      </label>
                    ))}
